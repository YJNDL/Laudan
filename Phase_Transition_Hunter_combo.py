...
- "Plan-C": if CONFIRMED_COMBOS is empty but TARGET_SGS is provided, the script
...
If you need full control (custom grids / direction generation / Plan-C details),
...
#       # NOTE: use the *canonical* irrep tags printed by this script (Mulliken labels like "A1@G").
#       #       IRxx-style internal ids are only for logging and are not accepted here.
#   "op_directions": [[1,1], [1,-1], ...],   # list[list[float]] each inner length = len(irrep_modes)
#       # NOTE: each direction vector is normalized internally; only ratios matter.
#       #       Actual amplitudes are set by rho (RHO_GRID) or PLAN_C_TEST_RHO.
# }
...
# Candidate pool size for random OP-direction sampling (internal use).
RANDOM_OP_DIRECTION_CANDIDATES = max(500, 50 * max(1, N_RANDOM_OP_DIRECTIONS_PER_COMBO))

# Combo selection policy (derived from CONFIRMED_COMBOS and TARGET_SGS):
#   1) If CONFIRMED_COMBOS is non-empty: scan only those combos (most reproducible).
#   2) Else if TARGET_SGS is non-empty: run Plan‑C to propose a small combo list.
#   3) Else: auto-generate combos for broad exploration.

PLAN_C_TEST_RHO = 0.6

# Adaptive combo size:
#   - Default (None): max combo size is inferred at runtime as n_modes = total number of
#     available displacement modes in the current `landau_basis`.
#   - For large systems, higher-order combos are *sampled* (not exhaustively enumerated) to
#     avoid combinatorial explosion. You can always force any specific k-mode combo by
#     putting it into CONFIRMED_COMBOS.
#   - Advanced: set an explicit integer cap if you really need it.
PLAN_C_MAX_COMBO_SIZE: Optional[int] = None  # None -> auto (= n_modes)
AUTO_COMBO_MAX_SIZE: Optional[int] = None   # None -> auto (= n_modes)

# OP-direction generation (do NOT hand-maintain direction pools):
#   - For a k-mode combo, a small deterministic base direction pool is generated by:
#       _base_dir_pool_for_dim(k, kind="plan_c" | "auto")
#   - If ENABLE_RANDOM_OP_DIRECTIONS is True, extra random directions are appended by:
#       build_op_directions_for_combo(...)
#   - For full manual control you can specify directions explicitly in:
#       CONFIRMED_COMBOS[i]["op_directions"]
#     where each inner vector has length = len(CONFIRMED_COMBOS[i]["irrep_modes"]).
...
def build_landau_basis_auto_pointgroup(
    ...
    for sig, by_p in sig_parity_modes.items():
        ...
        for ptag, vecs in by_p.items():
            ...
            # Log a stable internal id -> Mulliken label mapping for traceability.
...
def make_distorted_crystal(
    ...
    for (irrep_tag, midx), amp in mode_amplitudes.items():
        vecs = landau_basis.get(irrep_tag, None)
        if vecs is None:
            raise KeyError(
                f"Mode irrep '{irrep_tag}' not found in landau_basis. "
                f"Available: {list(landau_basis.keys())}"
            )
        if not (0 <= midx < len(vecs)):
            raise IndexError(f"Mode index out of range for {irrep_tag}: {midx}, n={len(vecs)}")
...
# ----------------------------------------------------------------------
# Combo generation helpers (adaptive)
# ----------------------------------------------------------------------
...
def _sample_mode_combinations(
    modes: List[Tuple[str, int]],
    k: int,
    max_samples: int,
    rng: np.random.Generator,
) -> List[Tuple[Tuple[str, int], ...]]:
    """Return up to max_samples unique k-combinations of modes.

    - If total combinations <= max_samples (or max_samples<=0), return all combinations.
    - Otherwise, randomly sample unique combinations (deterministic given rng seed).
    """
    n = len(modes)
    if k < 0 or k > n:
        return []
    ...
    if max_samples <= 0 or total <= max_samples:
        return list(combinations(modes, k))

    picked: Set[Tuple[Tuple[str, int], ...]] = set()
    # Try reasonably hard to get enough unique samples
    max_tries = max(1000, max_samples * 30)
    tries = 0
    while len(picked) < max_samples and tries < max_tries:
        tries += 1
        idx = np.sort(rng.choice(n, size=k, replace=False))
        combo = tuple(modes[int(i)] for i in idx)
        picked.add(combo)

    out = list(picked)
    # Deterministic ordering for logs / reproducibility
    out.sort(key=lambda t: (len(t), str(t)))
    return out


def _base_dir_pool_for_dim(dim: int, *, kind: str) -> List[List[float]]:
    """Return a tiny deterministic OP-direction pool for a combo of size `dim`.

    Why this exists:
    - Direction choices should be stable across runs (easy to reproduce / debug).
    - The base pool is intentionally *small* to avoid an explosion in trials.
    - If ENABLE_RANDOM_OP_DIRECTIONS is True, `build_op_directions_for_combo()` will append
      additional random directions on top of this base pool.

    kind:
      - "plan_c": slightly richer pool (used in the inexpensive Plan‑C confirmation stage)
      - "auto": smaller pool (used in the expensive full scan)
    """
    if dim <= 0:
        return []
    if dim == 1:
    ...
    dirs.append([1.0] * dim)
    # Alternating signs
    dirs.append([1.0 if (i % 2 == 0) else -1.0 for i in range(dim)])

    # A few "single-flip" patterns help explore different isotropy branches.
    # Keep it small to avoid exploding directions.
    flips = min(dim, 3 if kind == "auto" else 5)
...
def _plan_c_max_pairs_to_test(n_modes: int) -> int:
    """Sampling cap for 2-mode combos in Plan-C (to keep spglib calls bounded)."""
    # Plan-C is cheap-ish (no strain grid), so allow more.
    if SCAN_PROFILE == "fast":
        return min(8000, _n_choose_k(n_modes, 2))
    if SCAN_PROFILE == "dense":
        return min(20000, _n_choose_k(n_modes, 2))
    return min(12000, _n_choose_k(n_modes, 2))


def _auto_max_pairs_to_scan(n_modes: int) -> int:
    """Sampling cap for 2-mode combos in AUTO scanning (can be very expensive)."""
    # AUTO combos are scanned over (rho × strain grid × dirs), so keep smaller.
    if SCAN_PROFILE == "fast":
        return min(4000, _n_choose_k(n_modes, 2))
    if SCAN_PROFILE == "dense":
        return min(1200, _n_choose_k(n_modes, 2))
    return min(2000, _n_choose_k(n_modes, 2))


def _plan_c_samples_for_k(k: int, n_modes: int) -> int:
    """How many k-mode combos to sample in Plan-C for k>=3."""
    if k < 3:
        return 0
    # More aggressive for smaller k, decay with k.
    base = 80 if SCAN_PROFILE == "fast" else (240 if SCAN_PROFILE == "dense" else 140)
    # decay factor
    x = max(0, k - 3)
    n = int(round(base * (0.78 ** x)))
    # Always try at least a few for each k, but don't overdo it.
    return max(6, min(n, 500))


def _auto_samples_for_k(k: int, n_modes: int) -> int:
    """How many k-mode combos to sample in AUTO scanning for k>=3.

    Important: AUTO combos are expensive (scanned over rho/strain), so keep very small.
    """
    if k < 3:
        return 0
    base = 14 if SCAN_PROFILE == "fast" else (6 if SCAN_PROFILE == "dense" else 9)
    x = max(0, k - 3)
    n = int(round(base * (0.75 ** x)))
    # Ensure we at least have 1 sample for each k up to max_size (adaptive support),
    # but keep it tiny.
...
def plan_c_autofill_combos_bruteforce(
    ...
    Adaptive upgrade:
    - Supports k-mode combos up to PLAN_C_MAX_COMBO_SIZE.
    - If PLAN_C_MAX_COMBO_SIZE is None -> auto (= total number of modes).
    - For k>=3, we sample combos to avoid combinatorial explosion.
    """
    ...
    n_modes = len(modes_priority)
    max_size = PLAN_C_MAX_COMBO_SIZE if isinstance(PLAN_C_MAX_COMBO_SIZE, int) else n_modes
    max_size = int(max(1, min(max_size, n_modes)))

    print(f"[Plan-C] adaptive max_combo_size = {max_size} (n_modes={n_modes}, profile={SCAN_PROFILE})")

    found: Dict[Tuple[Tuple[Tuple[str, int], ...], Tuple[float, ...]], Set[int]] = {}

    rng = np.random.default_rng(RANDOM_OP_DIRECTION_SEED + 2025)
    ...
    # 1-mode (always exhaustive)
    for m in modes_priority:
        test_combo([m], [1.0])

    # k-mode for k>=2
    for k in range(2, max_size + 1):
        dir_pool = _base_dir_pool_for_dim(k, kind="plan_c")

        if k == 2:
            max_pairs = _plan_c_max_pairs_to_test(n_modes)
            combos_k = _sample_mode_combinations(modes_priority, 2, max_pairs, rng)
        else:
            n_sample = _plan_c_samples_for_k(k, n_modes)
            combos_k = _sample_mode_combinations(modes_priority, k, n_sample, rng)

        if not combos_k:
            continue

        for modes_k in combos_k:
            modes_list = list(modes_k)
            for d in dir_pool:
                # Ensure direction length matches combo size
                if len(d) != k:
                    continue
                test_combo(modes_list, d)
...
def auto_generate_combos(
    ...
    Adaptive upgrade:
    - Supports k-mode combos up to max_size.
    - If max_size is None -> auto (= total number of modes).
    - Exhaustive for 2-mode combos when small; samples when large.
    - For k>=3: sampled combos with small base direction pools (random dirs may be added later).
    """
    all_modes: List[Tuple[str, int]] = []
    for irrep_tag, vecs in landau_basis.items():
        for i in range(len(vecs)):
    ...
    max_eff = n_modes if (max_size is None or not isinstance(max_size, int)) else int(max_size)
    max_eff = int(max(2, min(max_eff, n_modes)))

    print(f"[AUTO_COMBO] adaptive max_combo_size = {max_eff} (n_modes={n_modes}, profile={SCAN_PROFILE})")

    rng = np.random.default_rng(RANDOM_OP_DIRECTION_SEED + 777)

    combos: List[Dict[str, Any]] = []

    # ---- 2-mode combos ----
    max_pairs = _auto_max_pairs_to_scan(n_modes)
    pair_combos = _sample_mode_combinations(all_modes, 2, max_pairs, rng)
    base_dirs_2 = _base_dir_pool_for_dim(2, kind="auto")

    for (m1, m2) in pair_combos:
        combos.append({
            "target_sgs": set(),  # unknown
            "irrep_modes": [m1, m2],
            "op_directions": [d[:] for d in base_dirs_2],
        })

    # ---- higher-order combos (k>=3) ----
    for k in range(3, max_eff + 1):
        n_sample = _auto_samples_for_k(k, n_modes)
        combos_k = _sample_mode_combinations(all_modes, k, n_sample, rng)
        if not combos_k:
            continue
        base_dirs_k = _base_dir_pool_for_dim(k, kind="auto")
        for modes_k in combos_k:
            combos.append({
                "target_sgs": set(),  # unknown
                "irrep_modes": [(m[0], int(m[1])) for m in modes_k],
                "op_directions": [d[:] for d in base_dirs_k],
            })
...
def build_combo_specs(
    ...
    Policy (simple + maintainable):
      1) If CONFIRMED_COMBOS is non-empty: scan only those combos.
      2) Else if TARGET_SGS is non-empty: run Plan‑C to propose a small combo list.
         If Plan‑C fails (or finds nothing), fall back to AUTO combos (never abort).
      3) Else: use AUTO combos directly.

    Notes:
    - Scanning supports arbitrary k-mode combos. The auto-generators will *sample*
      higher-order combos for large systems to keep runtime bounded.
    - Each combo dict must include:
        "irrep_modes": [(irrep_tag, mode_idx), ...]
        "op_directions": [[...], [...], ...]  # each inner vector length == len(irrep_modes)
    """
    # 1) Manual confirmed combos (most reproducible)
    if CONFIRMED_COMBOS:
        return CONFIRMED_COMBOS, True

    # 2) Plan‑C: only when user provided target SGs
    if TARGET_SGS:
        print(f"[INFO] CONFIRMED_COMBOS 为空，开始 Plan-C 自动枚举... target SGs = {TARGET_SGS}")
        try:
        ...
        print("==================================================================")
        print(f"[Plan-C] total combos found = {len(combos)}")
        print("==================================================================")
        print("[INFO] Plan-C 生成的 CONFIRMED_COMBOS 如下（可直接复制粘贴到脚本配置区）：")
        print("CONFIRMED_COMBOS = [")
        for c in combos:
            print(f"  {c},")
        print("]")
        print()
        ...
        print("[WARN] Plan-C 未找到可用 combos，将回退到 AUTO_COMBO 生成策略继续扫描（不再中止）。")
        combos_auto = auto_generate_combos(landau_basis, max_size=AUTO_COMBO_MAX_SIZE)
        return combos_auto, False

    # 3) Broad exploration
    print("[INFO] 未指定 TARGET_SGS 且 CONFIRMED_COMBOS 为空，使用 AUTO_COMBO 生成策略扫描。")
    combos_auto = auto_generate_combos(landau_basis, max_size=AUTO_COMBO_MAX_SIZE)
    return combos_auto, False
    ...
    for irrep_tag, vecs in landau_basis.items():
        for midx in range(len(vecs)):
            for amp, sa, sb, sc in product(AMP_GRID, STRAIN_A_GRID, STRAIN_B_GRID, STRAIN_C_GRID):
                ...
                hit = int(sg) in set(int(x) for x in TARGET_SGS)
        ...
        for dir_idx, d_raw in enumerate(dirs_to_scan, start=1):
            ...
            for rho, sa, sb, sc in product(RHO_GRID, STRAIN_A_GRID, STRAIN_B_GRID, STRAIN_C_GRID):
                ...
                if target_sgs and int(sg) in target_sgs:
                    hit = True
                if int(sg) in set(int(x) for x in TARGET_SGS):
                    hit = True
